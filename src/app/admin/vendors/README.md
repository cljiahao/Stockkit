...
- `page.tsx` — `AdminVendorsPage`: a Server Component that `requireAdmin()`-gates, fetches `listVendors()` (already sorted by triage urgency), and passes the rows to `VendorsTable` inside `@merqo/ui`'s `ElevatedCard` (2026-09-16, was stockkit-local).
