...
- `accept/actions.ts` — `acceptLegalTerms` server action. Re-checks for a
  ...
  body carrying the SHA-256 of that doc's `getLegalDocSource(...)`. Each call
  is independent, and merqo maps a duplicate `(email, doc_type, doc_version)`
  to a success, so a conflict on one doc never blocks the other. On success
  it primes the local `legal_check_state` cache to `is_current = true` and
